...
const NotifPopup = ({
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-3 w-full sm:w-80 bg-card rounded-lg shadow-2xl border border-border",
    ...
    >
      <div className="shrink-0">
        {n.actorImage ? (
          ...
          <div className="h-9 w-9 rounded-full bg-primary/20 flex items-center justify-center text-base">
            {TYPE_ICON[n.type] ?? "🔔"}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-medium text-card-foreground leading-snug">{n.message}</p>
        {n.cardTitle && (
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        ...
        {n.boardId && (
          <Link href={`/board/${n.boardId}`} className="text-muted-foreground/70 hover:text-primary transition-colors">
  ...
  return (
    <>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 h-4 min-w-4 px-0.5 bg-destructive text-destructive-foreground text-[10px] font-bold rounded-full flex items-center justify-center animate-pulse">
