...
const CalendarCard = ({ card }: CalendarCardProps) => {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        card.completed
          ...
          : "bg-secondary border border-border text-muted-foreground hover:border-muted-foreground"
      )}
    ...
    >
      ...
      {card.priority !== "NONE" && (
        <span className={cn("h-1.5 w-1.5 rounded-full shrink-0", PRIORITY_DOT_CLASS[card.priority])} />
      )}
...
export const CalendarView = ({ lists }: CalendarViewProps) => {
  ...
  return (
    <div className="flex flex-col h-full bg-background/90 rounded-lg overflow-hidden border border-border shadow-sm">
      ...
      {/* Day name headers */}
      <div className="grid grid-cols-7 bg-secondary border-b">
        {DAY_NAMES.map((d) => (
